...
## Phase 0 — INQUIRY (사후 정리, 2026-06-12)
- [x] 기본 3문 (문제 / 타깃 사용자 / 산출물 형태)
- [x] 레퍼런스 수집
- [x] 웹 리서치
- [x] 기능 후보 리스트업
- [x] 러프 UX 플로우
- [x] APPROVED (INQUIRY 개념 신설 전 진행 — 사후 정리)
...
- [x] UI Composition Decisions (Sidebar 6섹션, Center 멀티탭)
- [x] APPROVED (2026-05-22)
...
- [x] Inter-domain rule: orchestrator 직접 wire-up (ADR-018)
- [x] 단방향 .md → UI, AI 호출 없음
- [x] ADR-001~005 (TypeScript, AI 호출 X, 단방향, 통신, generic mode V4)
...
- [x] **단위 8: 가운데 webview 멀티탭** (2026-05-24)
  ...
  - [x] 8e: Preview 페이지 (디자인 시안 갤러리 + 토큰)
  - [x] 8f: Errors 페이지 (error.history.md 렌더 / 없으면 생성)
  - [x] 8g: styles.css 멀티탭 + 탭 시스템 + 글래스 카드
...
- [x] 단위 10: INQUIRY 연동 + Guide 탭 + Plan "지금 만드는 것" 칸 + 미결 게이트 (2026-06-12)
...
## Phase 5.7 — QA (2026-06-09)
- [x] 빌드/컴파일 — tsc --noEmit 에러 0, esbuild 번들 OK
- [x] 파서 전수 — state.md → BlueprintState, phase·status·counters·빈입력 회복성
- [x] Webview 탭 렌더러 — Plan/Spec/Preview/QA/Errors/Guide 무결 (렌더 누출 0건)
- [x] QA 탭 자체 — PASS/WARN/FAIL 집계·verdict·노션식 접이식 토글
- [x] 패키징 — vsce package 성공, .qa-tmp 누출 발견·수정(.vscodeignore)
- [x] 재실행 하네스 — `npm run qa` (171/171 PASS, exit 0)
- [x] docs/qa.report.md 기록 → QA 탭 단방향 렌더
- [ ] (선택) Extension Host F5 dogfooding — webview 탭 실측

## Phase 6 — SHIP (2026-05-24)
- [x] CHANGELOG.md 작성
- [x] package.json v0.0.1 → v0.1.0
- [x] /ship 수동 — 최종 .vsix 빌드
- [x] git commit + push
- [x] git tag v0.1.0 + push tag
- [ ] (선택) GitHub Release 페이지 만들기

## Phase 7 — POST-SHIP (2026-05-24 — 일부 SHIP과 동시)
- [x] README.md 반영 (설치/사용/dev 흐름)
