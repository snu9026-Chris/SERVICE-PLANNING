...
## Next action
v0.14.0 설치+리로드 후 Guide 탭·사이드바 미결 배지·phase 키보드 내비 확인. 백로그: mermaid 도식화, generic mode(V4), 1주 dogfooding.
...
- 2026-06-12: Phase 0 INQUIRY 신설 — blueprint 스킬 v2(INQUIRY 절차+RESUME 자동백필, 전체 phase +1, 별도 public repo) + 대시보드 연동(spec/panel/extension)·docs/design 라이브갱신. ADR-017 Guide 탭(Errors 오른쪽, 기능·단계별·변경이력 내장, .md→UI 예외). + 완결성 정리(문서 동기화·ADR-018·헬퍼 통일·사이드바 a11y/미결 배지). 하네스 175/175. (v0.12.0→0.14.0)
