...
- **가운데 webview** (6페이지 멀티탭):
  - **Plan** — `plans/roadmap.md` 풀-너비 + "지금 만드는 것"(프로젝트 한 줄 설명) + 현재 phase 강조 + 미결 항목 경고 배너
  - **Spec** — INQUIRY/PRODUCT/DESIGN/ARCHITECTURE 산출물을 폴더 탐색기처럼 (좌 트리 / 우 풀-너비 렌더)
  - **Preview** — `docs/design/` 디자인 시안 갤러리(인라인 iframe) + DESIGN.md 색·폰트 토큰 자동 추출
...
## 설치
1. https://github.com/snu9026-Chris/SERVICE-PLANNING/raw/main/blueprint-dashboard-0.13.0.vsix 다운로드
2. Antigravity → `Ctrl+Shift+P` → "Install from VSIX..." → 다운받은 파일 → Reload Window

## 전체 워크플로 설치 (다른 컴퓨터)
확장만이 아니라 `/blueprint` 워크플로까지 새 머신에서 쓰려면:
- **확장**: 이 repo clone → `blueprint-dashboard-0.13.0.vsix` 설치
...
4. webview 상단 탭으로 Plan / Spec / Preview / QA / Errors / Guide 전환
...
npm run qa             # 자동 하네스 (171 단언, exit 0/1)
...
src/
  parser/              ← state.md 파싱 + BUILD TARGET 감지 (순수)
